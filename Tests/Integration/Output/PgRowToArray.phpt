...
final class PgRowToArray extends TestCase\PostgresDatabase {
	public function testConvertingToArray() {
		...
		Assert::same(
			['name' => 'Dom', 'race' => 'human'],
			(new Output\PgRowToArray($this->connection, '(Dom,human)', 'person'))->value()
		);
	}

	public function testThrowingOnUnknownType() {
		$ex = Assert::exception(function() {
			(new Output\PgRowToArray($this->connection, '(Dom,human)', 'xxx'))->value();
	...
	public function testThrowingOnNotProperUseOfRow() {
		...
		(new Storage\NativeQuery($this->connection, 'CREATE TYPE person AS (name TEXT, race TEXT)'))->execute();
		$ex = Assert::exception(function() {
			(new Output\PgRowToArray($this->connection, '(Dom,human,idk)', 'person'))->value();
	...
	public function testThrowingOnNotProperUseOfRowAsTable() {
		...
		$ex = Assert::exception(function() {
			(new Output\PgRowToArray($this->connection, '(Dom,human,idk)', 'person_table'))->value();
	...
	public function testConvertingArrayOfRowsArray() {
		...
		Assert::same(
			...
			(new Output\PgRowToArray($this->connection, '{"(Dom,human)","(Dan,master)"}', 'person[]'))->value()
		);
	}
	...
	public function testConvertingTableTypeToArray() {
		...
		Assert::same(
			['name' => 'Dom', 'race' => 'human'],
			(new Output\PgRowToArray($this->connection, '(Dom,human)', 'person_table'))->value()
		);
	}
	...
	public function testConvertingNestedType() {
		...
		Assert::same(
			...
			(new Output\PgRowToArray($this->connection, '("(10,mm)",Dom)', 'person_table'))->value()
		);
	}
}
